...
function showMovieComments(element) {
    ...
    document.getElementById("userReview").innerHTML='	<button type="button" class="btn btn-primary" id="newComment" data-toggle="modal" data-target="#newCommentModal" 	data-dismiss="modal" onClick="newComment()">New Comment</button>';
    }
    document.getElementById("emptyComment").innerHTML = "No review yet. Create one now";
    var item = element.getAttribute("item");
    currentIndex = item;

    document.getElementById("review").textContent = "Review for " + movie_array[item].restaurant;
    document.getElementById("commentBody").textContent = "";

    for (var i = 0; i < comment_array.length; i++) {
        if (comment_array[i].restaurant_id == movie_array[item].id) {
            ...
            var html = '<div class="text-center" style="width:100%;">                                                           \
                            <div class="card">                                                                                  \
                                <div class="card-body">                                                                         \
                                    ...
                                    <img width='30' height='30' src='css/image/upload/"+comment_array[i].image+"'>                          \
           ...
           if(sessionStorage.getItem('user')== comment_array[i].username){
...
function addComment() {
    ...
    comment.restaurant_id = movie_array[currentIndex].id; // Movie ID is required by server to create new comment 
    comment.user_id = sessionStorage.getItem('id'); // Movie title is required by server to create new comment
